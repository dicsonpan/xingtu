...
import { testAIConnection, AVAILABLE_MODELS } from '../_lib/ai';
...
// ============================================================
// AI 配置管理 (Cloudflare Workers AI)
// ============================================================

// 获取 AI 配置
app.get('/ai-config', async (c) => {
  const row = await c.env.DB
    .prepare('SELECT model, updated_at FROM system_settings WHERE id = 1')
    .first<any>();

  return c.json({
    success: true,
    data: {
      model: row?.model || '@cf/qwen/qwen3-30b-a3b-fp8',
...
app.put('/ai-config', async (c) => {
  ...
  if (!model) {
    return c.json({ success: false, error: '请选择模型' }, 400);
  }
  ...
  await c.env.DB
    .prepare(
      `INSERT INTO system_settings (id, model, updated_at, updated_by)
       VALUES (1, ?, datetime('now'), ?)
       ON CONFLICT(id) DO UPDATE SET model = excluded.model, updated_at = datetime('now'), updated_by = excluded.updated_by`
    )
    .bind(model.trim(), adminId)
    .run();

  return c.json({ success: true, data: { message: 'AI 模型配置已更新' } });
});

// 测试 AI 连接
app.post('/ai-config/test', async (c) => {
  const result = await testAIConnection(c.env.AI, c.env.DB);
